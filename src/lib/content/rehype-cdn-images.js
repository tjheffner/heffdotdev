...
export default function rehypeCdnImages() {
  return (tree) => {
    visit(tree, { type: 'element', tagName: 'img' }, (node) => {
      // these pages are image heavy and the raw GitHub attachments are
      // full-size camera output.
      // data-lightbox-src is @hyzer-labs/ui's full-size override. it names the
      // large variant lightboxGroup should open, so the viewer doesn't enlarge
      // whichever thumbnail the srcset picked.
