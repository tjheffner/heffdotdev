...
export const n36 = (value: number, scale = 1): string => Math.round(value * scale).toString(36);
...
export const packHex = (list: string[]): string => list.map((c) => c.replace(/^#/, '')).join('');
export const unpackHex = (s: string): string[] => (s.match(/.{6}/g) || []).map((h) => `#${h}`);
