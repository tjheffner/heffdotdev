...
function bitrateFor(w: number, h: number, fps: number) {
  ...
  return Math.min(16_000_000, Math.max(2_000_000, Math.round(w * h * fps * 0.13)));
}

// True if the browser can encode H.264 at this size via WebCodecs.
async function mp4Supported(width: number, height: number, fps: number): Promise<boolean> {
...
async function encodeMp4(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, opts: RecordOptions) {
  ...
  download(new Blob([muxer.target.buffer], { type: 'video/mp4' }), `${opts.filename}.mp4`);
}

async function encodeWebm(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, opts: RecordOptions) {
...
export function viewportClipDims(long = 960): { width: number; height: number } {
...
export async function recordViewportClip(opts: {
  ...
  fps: number;
  filename: string;
  draw: (ctx: CanvasRenderingContext2D, i: number, W: number, H: number, frames: number) => void;
  onProgress?: (fraction: number) => void;
  before?: () => void;
  after?: () => void;
  long?: number;
